...
The full-corpus runner admits all 517,401 records by their parsed outer dates, including anomalous dates that must not be treated as reliable historical delivery. Work is distributed across 130 batches of at most 4,000 arrivals, with up to ten investigator dispatches per batch and a shared total budget. Sender partitions use deterministic hash sampling rather than the first messages in each batch. Arrived-only body matching prefilters registered watches before exact segment validation; this makes watching the full corpus feasible without paying for each message. Models read bounded samples and their chosen retrievals, not every admitted email. See the full-corpus protocol and report for measured coverage and results.
